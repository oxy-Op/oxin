...
class ServerInfo(commands.Cog):
  ...
  async def serverinfo(self,ctx):
    try:
      ...
      embed.add_field(name="Channels Count",value=len(server.text_channels))
      embed.add_field(name="Voice channels count",value=len(server.voice_channels))
      ...
      embed.add_field(name="Total Boosts",value=server.premium_subscription_count)
      embed.add_field(name="Boost Level",value=server.premium_tier)
      embed.add_field(name="Features",value=server.features,inline=False)
      embed.add_field(name="Server Created at ",value=server.created_at)
      embed.set_thumbnail(url=server.icon_url)
      embed.set_footer(text="Message Requested by "+ ctx.author.name +"#"+ ctx.author.discriminator,icon_url = ctx.author.avatar_url)
